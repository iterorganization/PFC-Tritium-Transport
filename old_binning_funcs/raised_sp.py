...
from bg_fluxes_binned import (
    remove_structure_points_soledge,
    read_wall_soledge,
    read_dat_file,
    load_geometry,
    create_bins,
...
def read_DINA(DINA_data):
    ...
    DINA = np.loadtxt(lines, skiprows=1)  # TODO: skiprows not needed?
...
def read_div_solps(
    ...
    # first, remove first row since we don't have that line of data in solps
    ...
    header = "r1,z1,r2,z2,rc,zc,xc,ne,Te,Ti,Tn,flxi,flxn,Wtot,Wion"
    ...
    inner_lines_script[-1][4] = np.mean(
        [inner_lines_script[-1][0], inner_lines_script[-1][2]]
    )  # rc
    inner_lines_script[-1][5] = np.mean(
        [inner_lines_script[-1][1], inner_lines_script[-1][3]]
    )  # zc
    ...
    inner_lines_script[-1][10] = inner_particle_flux[-1][
        2
    ]  # Tn, setting them roughly equal for now
    ...
    outer_lines_script[-1][4] = np.mean(
        [outer_lines_script[-1][0], outer_lines_script[-1][2]]
    )  # rc
    outer_lines_script[-1][5] = np.mean(
        [outer_lines_script[-1][1], outer_lines_script[-1][3]]
    )  # zc
    ...
    outer_lines_script[-1][10] = outer_power_loads[-1][
        2
    ]  # Tn, setting them roughly equal for now
    ...
    return inner_lines_script, outer_lines_script, data_div, header
...
def find_xc(data, div_bins, inner_flag=True):
    ...
    if inner_flag:
        ...
        # div bin centers
        div_bin_centers = [
            (np.mean(z_bin), np.mean(r_bin)) for z_bin, r_bin in div_bins[25:]
        ]
        ...
        for idx, (
            ((z_start, z_end), (r_start, r_end)),
            (z_center, r_center),
        ) in enumerate(zip(div_bins[25:], div_bin_centers)):
            dist = np.sqrt((z_start - z_center) ** 2 + (r_start - r_center) ** 2)
            xc_bins[idx + 25].append(x_ref + dist)
            x_ref += 2 * dist

    else:
        # div bin centers
        div_bin_centers = [
            (np.mean(z_bin), np.mean(r_bin)) for z_bin, r_bin in div_bins[:15]
        ]
        ...
        for idx, (
            ((z_start, z_end), (r_start, r_end)),
            (z_center, r_center),
        ) in enumerate(zip(div_bins[:15], div_bin_centers)):
            dist = np.sqrt((z_start - z_center) ** 2 + (r_start - r_center) ** 2)
            xc_bins[idx].append(x_ref + dist)
            x_ref += 2 * dist
...
def plot_sp_time(time, sp, name):
    ...
    plt.savefig("old_binning_funcs/Dina_Raised_" + name + "_on_Time")
...
def time_dep(
    ...
    time = list(range(ramp_up + steady_state + ramp_down + 1))
        ...
        if t > ramp_up + constant_in_time and t < ramp_up + steady_state:
            ...
            if inner:
                for x in xc_solps:
                    if (
                        x > xc_bins[-5]
                    ):  # slow slope -- TODO: this never actually goes through because the solps data x-coordinates are all always smaller than this value
                        shift = (
                            max_sweep_x
                            + slopeslow * (t - (ramp_up + constant_in_time + 1))
                            + x
                        )
                        ...
                        shift = (
                            max_sweep_x
                            + slopefast * (t - (ramp_up + constant_in_time + 1))
                            + x
                        )
                        xc_long_solps.append(shift)
    ...
    if inner:
        ...
        plt.savefig("old_binning_funcs/Profile_Inner")
    else:
        plt.title("SOLPS Outer Target Heat Load Profile")
        plt.savefig("old_binning_funcs/Profile_Outer")
    ...
    for idx, array in enumerate(master_list):
        plt.plot(
            master_list[idx][0],
            ion_flux,
            label="SOLPS Ion Flux Profile at " + str(idx) + "s",
        )
    ...
    if inner:
        ...
        plt.savefig("old_binning_funcs/Ion_Flux_Inner")
    else:
        plt.title("SOLPS Outer Target Ion Flux Profile")
        plt.savefig("old_binning_funcs/Ion_Flux_Outer")
    ...
    for idx, array in enumerate(x_shifted):
        for array2 in array:
            plt.plot(
                array2, atom_flux, label="SOLPS Ion Flux Profile at " + str(idx) + "s"
            )
    ...
    if inner:
        ...
        plt.savefig("old_binning_funcs/Atom_Flux_Inner")
    else:
        plt.title("SOLPS Outer Target Atom Flux Profile")
        plt.savefig("old_binning_funcs/Atom_Flux_Outer")
    ...
    for idx, array in enumerate(x_shifted):
        for array2 in array:
            plt.plot(
                array2, ion_E, label="SOLPS Ion Energy Profile at " + str(idx) + "s"
            )
    ...
    if inner:
        ...
        plt.savefig("old_binning_funcs/Ion_Energy_Inner")
    else:
        plt.title("SOLPS Outer Target Ion Energy Profile")
        plt.savefig("old_binning_funcs/Ion_Energy_Outer")
    ...
    for idx, array in enumerate(x_shifted):
        for array2 in array:
            plt.plot(
                array2, atom_E, label="SOLPS Ion Energy Profile at " + str(idx) + "s"
            )
    ...
    if inner:
        ...
        plt.savefig("old_binning_funcs/Atom_Energy_Inner")
    else:
        plt.title("SOLPS Outer Target Atom Energy Profile")
        plt.savefig("old_binning_funcs/Atom_Energy_Outer")
...
def bin_SOLEDGE_fluxes(data_wall, data_div, bins, div_bins):
    ...
    alpha_ionw = np.full(
        len(ion_fluxw), 6.0e01
    ).tolist()  # approximating as 40 degrees for all points
    alpha_atomw = np.full(
        len(atom_fluxw), 4.5e01
    ).tolist()  # approximating as perpendicular for all points
    ...
    total_indices_covered = sum(
        len(bin_indices) for bin_indices in indices_covered_by_bin
    )
...
def bin_SOLPS_fluxes(div_bins, master_list, xc_bins, t, inner=True):
    ...
    alpha_iond = np.full(
        len(ion_fluxd), 6.0e01
    ).tolist()  # approximating as 40 degrees for all points
    alpha_atomd = np.full(
        len(atom_fluxd), 4.5e01
    ).tolist()  # approximating as perpendicular for all points
    ...
    if inner:
        if t >= ramp_up + constant_in_time + 1 and t < ramp_up + steady_state + 1:
            ...
            with open(
                filename
            ) as file:  # indexing matches at t=101 (means t=1 for angle coeffs)
                lines = [line for line in file]
...
if __name__ == "__main__":
    ...
    DINA_inner = read_DINA("old_binning_funcs/RISP_excel.txt")
    DINA_outer = read_DINA("old_binning_funcs/ROSP_excel.txt")
    ...
    inner_data, outer_data, data_div, header = read_div_solps(
        "raw_data/sol_simus/solps_data/raised_SP_solps/fp_tg_i.123102.dat",
        "raw_data/sol_simus/solps_data/raised_SP_solps/fo_tg_o.123102.dat",
        "raw_data/sol_simus/solps_data/raised_SP_solps/ld_tg_i.123102.dat",
        "raw_data/sol_simus/solps_data/raised_SP_solps/ld_tg_o.123102.dat",
        "raw_data/sol_simus/inner_target.shot123102.run1.dat",
        "raw_data/sol_simus/outer_target.shot123102.run1.dat",
    )
    # np.savetxt(
    #     "inner.txt",
    #     inner_data,
    ...
    div_z_start, div_r_start, div_z_end, div_r_end = load_geometry(
        "old_binning_funcs/RISPDivpanelscorners.txt", wall=False
    )
    z_coord_start, r_coord_start, z_coord_end, r_coord_end = load_geometry(
        "old_binning_funcs/FWpanelcorners.txt"
    )
    ...
    outer_bins = list(range(18, 18 + 15))
    ...
    angle_coeffs_matlab = [
        "raw_data/angles_on_raised_div/angle_coeffs_t1.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t2.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t3.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t4.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t5.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t6.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t7.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t8.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t9.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t10.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t11.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t12.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t13.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t14.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t15.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t16.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t17.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t18.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t19.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t20.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t21.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t22.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t23.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t24.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t25.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t26.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t27.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t28.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t29.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t30.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t31.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t32.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t33.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t34.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t35.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t36.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t37.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t38.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t39.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t40.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t41.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t42.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t43.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t44.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t45.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t46.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t47.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t48.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t49.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t50.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t51.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t52.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t53.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t54.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t55.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t56.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t57.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t58.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t59.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t60.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t61.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t62.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t63.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t64.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t65.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t66.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t67.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t68.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t69.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t70.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t71.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t72.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t73.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t74.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t75.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t76.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t77.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t78.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t79.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t80.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t81.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t82.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t83.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t84.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t85.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t86.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t87.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t88.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t89.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t90.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t91.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t92.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t93.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t94.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t95.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t96.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t97.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t98.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t99.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t100.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t101.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t102.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t103.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t104.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t105.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t106.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t107.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t108.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t109.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t110.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t111.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t112.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t113.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t114.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t115.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t116.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t117.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t118.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t119.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t120.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t121.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t122.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t123.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t124.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t125.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t126.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t127.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t128.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t129.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t130.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t131.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t132.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t133.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t134.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t135.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t136.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t137.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t138.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t139.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t140.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t141.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t142.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t143.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t144.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t145.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t146.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t147.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t148.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t149.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t150.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t151.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t152.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t153.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t154.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t155.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t156.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t157.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t158.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t159.txt",
        "raw_data/angles_on_raised_div/angle_coeffs_t160.txt",
    ]

    # inner heat profile
    x_inner, master_list_inner = time_dep(
        lowest_in,
        inner_heat,
        inner_data,
        slope_in,
    ...
    # outer heat profile
    x_outer, master_list_outer = time_dep(
        lowest_out,
        outer_heat,
        outer_data,
        slope_out,
    ...
    # np.savetxt("outer_master.txt", master_list_outer[200][-1])
    ...
    outer_bin_ids = list(range(18, 33))
    ...
    for t in time:  # t is equivalent to index in master_list!
        ...
        (
            ...
            inner_bins, master_list_inner[t], inner_xc_bins, t, inner=True
        )

        header = (
            "Bin_Index,Flux_Ion,Flux_Atom,E_ion,E_atom,alpha_ion,alpha_atom,heat_total"
        )
        data_to_save = np.array(
            [
                [
                ...
                for bin_index, ion_flux, atom_flux, E_ion, E_atom, alpha_ion, alpha_atom, heat in zip(
                    inner_bin_ids,
                    binned_div_ion,
                    binned_div_atom,
                    div_E_ion,
        ...
        np.savetxt(
            "RISP_data/time" + str(t) + ".dat",
            data_to_save,
            delimiter=",",
            header=header,
            comments="",
            fmt=["%d"] + ["%.18e"] * (data_to_save.shape[1] - 1),
        )  # , fmt=['%d', '%f', '%f', '%f', '%f', '%f', '%f', '%f'])
        ...
        (
            ...
            outer_bins, master_list_outer[t], outer_xc_bins, t, inner=False
        )
