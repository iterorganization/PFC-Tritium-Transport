...
def read_div_solps(
    ...
    inner_lines_script[-1][10] = inner_power_loads[-1][
        3
...
def load_geometry(filename, wall=True):
    ...
    if wall:
        z_coord_start = geometry[:, 0] * 1e-3  # convert mm to m

        # r-coordinates are the second column in data, convert to list
        r_coord_start = geometry[:, 1] * 1e-3  # convert mm to m

        z_coord_end = geometry[:, 2] * 1e-3  # convert mm to m

        r_coord_end = geometry[:, 3] * 1e-3  # convert mm to m

    else:
        z_coord_start = geometry[:, 0]
        r_coord_start = geometry[:, 1]
        z_coord_end = geometry[:, 2]
        r_coord_end = geometry[:, 3]

    return z_coord_start, r_coord_start, z_coord_end, r_coord_end
    ...
    for idx in range(len((r1))):
        ...
        if z1[idx] >= -3.051:
            ...
            # second, we want to keep everything above -3.760 and to the right of 4.500
                ...
                z1[idx] >= -3.790 and r1[idx] <= 5.000
            ):  # adjusted based on data i see in trial case
                data_wall_clean.append((data_wall[idx].tolist()))
                removed_idx.append(idx)
...
def create_bins(z_start, r_start, z_end, r_end):
    """
    ...
    for coord in range(len(z_start)):
        z_s = z_start[coord]
        z_e = z_end[coord]
        r_s = r_start[coord]
        r_e = r_end[coord]
...
def binned_flux_plot(
    ...
    for bin_indices, ion_flux, atom_flux in zip(
        indices_covered_by_bin[: len(bins)], ion_flux_total, atom_flux_total
    ):
        for idx in bin_indices:
            expanded_ion[idx] = (
                ion_flux  # all values in the bin are set to the mean flux for that bin
            )
            expanded_atom[idx] = atom_flux

    # divertor indices expansion second, which are the indices after the wall bins
    for bin_indices, ion_flux, atom_flux in zip(
        indices_covered_by_bin[len(bins) :],
        ion_flux_total[len(bins) :],
        atom_flux_total[len(bins) :],
    ):
        for idx in bin_indices:
            expanded_ion[idx] = (
                ion_flux  # all values in the bin are set to the mean flux for that bin
    ...
    # plt.tight_layout()
    # plt.savefig('old_binning_funcs/Binned_Fluxes.png')
    ...
    plt.tight_layout()
    plt.savefig("old_binning_funcs/Binned_Fluxes_v._Distance.png")
    ...
    for bin_indices, heat in zip(indices_covered_by_bin[: len(bins)], heat_total):
        for idx in bin_indices:
            expanded_heat[idx] = (
                heat  # all values in the bin are set to the mean flux for that bin
            )

    # divertor indices expansion second, which are the indices after the wall bins
    for bin_indices, heat in zip(
        indices_covered_by_bin[len(bins) :], heat_total[len(bins) :]
    ):
        for idx in bin_indices:
            expanded_heat[idx] = (
                heat  # all values in the bin are set to the mean flux for that bin
    ...
    # plt.tight_layout()
    plt.savefig("old_binning_funcs/Binned_Heats_v._Distance.png")
...
def bin_2D_plots(bins, div_bins, data_div, data_wall, div_indices):
    ...
    for i in range(1, len(wall_r)):
        ...
        plt.text(
            mid_r, mid_z, str(i), fontsize=25, ha="center", va="center", color="green"
        )
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig("old_binning_funcs/Main_Chamber_Bins")
...
if __name__ == "__main__":
    ...
    data = read_dat_file("old_binning_funcs/Background_Flux_Data")

    inner_data, outer_data, data_div = read_div_solps(
        "raw_data/sol_simus/solps_data/fp_tg_i.2481.dat",
        "raw_data/sol_simus/solps_data/fp_tg_o.2481.dat",
        "raw_data/sol_simus/solps_data/ld_tg_i.2481.dat",
        "raw_data/sol_simus/solps_data/ld_tg_o.2481.dat",
        "raw_data/sol_simus/inner_target.shot122481.run1.dat",
        "raw_data/sol_simus/outer_target.shot122481.run1.dat",
    )
    ...
    (
        ...
        wall_heat,
        heat_ion,
    ) = bin_fluxes(data, data_wall, data_div, bins, div_bins)
    ...
    data_to_save = np.array(
        [
            ...
            for (
                (z_coord, _),
                (r_coord, _),
            ), ion_flux, atom_flux, E_ion, E_atom, alpha_ion, alpha_atom, heat in zip(
                bins,
                binned_ion_flux,
                binned_atom_flux,
                wall_E_ion,
    ...
    np.savetxt(
        "Wall_Flux_Data.dat", data_to_save, delimiter="\t", header=header, comments=""
    )
    ...
    data_to_save = np.array(
        [
            ...
            for bin_index, ion_flux, atom_flux, E_ion, E_atom, alpha_ion, alpha_atom, heat, heat_ion_val in zip(
                list(range(len(bins) + len(div_bins))),
                ion_flux_total,
                atom_flux_total,
                E_ion_total,
                E_atom_total,
    ...
    case_files = [
        "raw_data/wdn-data/i-wdn-0003-2481-00d-Ne.mrt.wall_flux",
        "raw_data/wdn-data/i-wdn-0003-2481-00g-Ne.mrt.wall_flux",
        "raw_data/wdn-data/i-wdn-0003-2481-00k-Ne.mrt.wall_flux",
        "raw_data/wdn-data/i-wdn-0003-2481-00m-Ne.mrt.wall_flux",
        "raw_data/wdn-data/i-wdn-0003-2481-01d-Ne.mrt.wall_flux",
        "raw_data/wdn-data/i-wdn-0003-2481-01g-Ne.mrt.wall_flux",
        "raw_data/wdn-data/i-wdn-0003-2481-01k-Ne.mrt.wall_flux",
        "raw_data/wdn-data/i-wdn-0003-2481-01m-Ne.mrt.wall_flux",
    ]
    ...
    if all(case is not None for case in cases):
        ...
        # plotting
        avg_ion_flux, avg_neutral_flux = plot_data(
            wall_index, r1, z1, ion_fluxes, neutral_fluxes, case_labels
        )
        binned_flux_plot(
            avg_ion_flux,
            avg_neutral_flux,
