# Modifying make_ter_bins to ignore Boron for now, full Tungsten bins will be considered preliminarily in order to extend simulations to full DT-1 campaign 
...
for bin_index in [10, 11, 12]:
    ...
    FW_bins.get_bin(bin_index).low_wetted_subbin.thickness = 12e-3
    FW_bins.get_bin(bin_index).low_wetted_subbin.material = "W"
    FW_bins.get_bin(bin_index).shadowed_subbin.thickness = 12e-3
    FW_bins.get_bin(bin_index).shadowed_subbin.material = "W"
...
for bin_index in [13, 14]:
    ...
    FW_bins.get_bin(bin_index).shadowed_subbin.thickness = 6e-3
    FW_bins.get_bin(bin_index).shadowed_subbin.material = "W"
...
for bin_index in [15]:
    ...
    FW_bins.get_bin(bin_index).high_wetted_subbin.material = "W"
    FW_bins.get_bin(bin_index).low_wetted_subbin.thickness = 6e-3
    FW_bins.get_bin(bin_index).low_wetted_subbin.material = "W"
    FW_bins.get_bin(bin_index).shadowed_subbin.thickness = 6e-3
...
# ------- DIV BINS -------
# I still need to remove boron from the divertor bins and check for thickness of W tiles. 6e-3?
